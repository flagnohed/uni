...
namespace details{
    ...
    auto enumerate(T const& container, U&& callable, int, int) 
        -> decltype(callable(*std::begin(container), 0, container.size()), std::declval<void>()){
        int i{};
        for (auto&& e : container) {
            callable(e, i, container.size());
            i++;
        }
    }

    template<typename T, typename U>
    auto enumerate(T const& container, U&& callable, int, float)
    ...
    template<typename T, typename U>
    auto enumerate(T const& container, U&& callable, float, float)
...
template<typename T, typename U>
void enumerate(T const& container, U&& callable) {
    details::enumerate(container, std::forward<U>(callable), 0, 0);
}
